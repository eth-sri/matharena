...
class MajorityJudge(BaseJudge):
    ...
    def prepare_clients(self):
        self.clients = []
        for model_config in self.solver_config["model_configs"]:
            model_config = yaml.safe_load(open(os.path.join("configs", "models", model_config + ".yaml")))
    ...
    def solve(
        ...
        for client in self.clients:
            conversation = normalize_conversation(self._query(client, [{"role": "user", "content": prompt}]))
            text = conversation[-1]["content"]
            outputs.append(text)

        outputs_as_str = "\n\n".join(f"### Output from model {i + 1} ###\n{text}" for i, text in enumerate(outputs))
        final_prompt = self.solver_config["main_judge_prompt"].format(
            problem_statement=problem_statement,
            original_problem_statement=original_problem_statement,
            guidelines=guidelines,
            student_answer=student_answer,
            ground_truth_solutions=gt_text,
            model_outputs=outputs_as_str,
        )

        conversation = normalize_conversation(self._query(self.main_client, [{"role": "user", "content": final_prompt}]))
        text = conversation[-1]["content"]
